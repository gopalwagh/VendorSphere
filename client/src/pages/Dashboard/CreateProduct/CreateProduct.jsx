...
const CreateProduct = () => {
  ...
  const handleOptimizeDescription = async () => {
    if (!title || !category) {
      toast.error("Please enter a title and category to optimize the description.");
      return;
    }
    setIsOptimizing(true);
    
    // We pass the raw description as "rawNotes" to the backend
    const result = await dispatch(optimizeDescriptionThunk({ 
      title, 
      category, 
    ...
    if (result.success) {
      const fullText = result.data.optimizedDescription || "";
      const tags = result.data.seoTags || [];
      ...
      // Stagger tags appearance
      const revealedTags = [];
      for (let i = 0; i < tags.length; i++) {
        revealedTags.push(tags[i]);
        setSeoTags([...revealedTags]);
        await new Promise(resolve => setTimeout(resolve, 150)); 
      }
      
      toast.success("Description optimized successfully!");
    } else {
      toast.error(result.message || "Failed to optimize description.");
    }
              ...
              <div className="cp-form-section">
                ...
                <div className="cp-form-group">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.375rem' }}>
                    ...
                    <button 
                      type="button" 
                      className="cp-optimize-btn" 
                      onClick={handleOptimizeDescription}
                      disabled={isOptimizing}
                      title="Optimize Description & Generate SEO Tags with AI"
                    >
                      {isOptimizing ? (
                        <>
                          <div className="cp-optimize-spinner"></div>
                          Optimizing...
                        </>
                      ) : (
                        "Optimize with AI ✨"
                  ...
                  <textarea
                    ...
                    rows={5}
                    disabled={isOptimizing}
                    className={isOptimizing ? "cp-textarea-generating" : "cp-textarea"} 
                  />
                  {seoTags.length > 0 && (
                    <div className="cp-seo-tags-container">
                      <label>SEO Tags generated by AI:</label>
                      <div className="cp-tags-list">
                        {seoTags.map((tag, idx) => (
                          <span key={idx} className="cp-tag-badge">
                            {tag}
                            <button 
                              type="button" 
